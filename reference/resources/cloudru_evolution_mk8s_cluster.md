...
- `nodes_subnet_cidr` (String) Адрес подсети узлов в формате RFC1918. Если указать это значение, то сеть будет создана автоматически.
- `nodes_subnet_id` (String) Идентификатор существующей подсети узлов.
...
- `size` (Number) Размер подключаенного диска в ГБ.
...
- `network` (String) Тип сети, которой принадлежит адрес. Возможные значения: * `NETWORK_TYPE_PRIVATE` — cерая пользовательская сеть (RFC1918). * `NETWORK_TYPE_PUBLIC` — публичная пользовательская сеть (интернет).
